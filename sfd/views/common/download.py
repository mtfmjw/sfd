...
class DownloadMixin:
    ...
    def download_file(self, request, queryset=None) -> HttpResponse:
        ...
        response["Content-Disposition"] = f'attachment; filename="{self.get_csv_file_name(request)}"'
...
class BaseModelDownloadMixin(DownloadMixin):
    def get_download_columns(self, request) -> dict[str, str]:
        ...
        filtered_columns = {k: v for k, v in columns.items() if k not in base_model_fields}
