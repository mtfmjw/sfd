        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    logger.info(f"Uploaded {file.name} successfully. Inserted: {self._total_inserted}, Updated: {self._total_updated}")

                    # Add success message for both HTMX and regular requests
                    self.message_user(  # type: ignore
                        request,
                        _("Uploaded %(file_name)s successfully.\n Inserted: %(inserted)s rows, Updated: %(updated)s rows.")
                        % {"file_name": file.name, "inserted": self._total_inserted, "updated": self._total_updated},
                        level="success",
                    )
                    ...
                    self.message_user(  # type: ignore
                        request,
                        _("An unexpected error has occurred. Please contact your system administrator and provide this error key [%(error_key)s].")
                        % {"error_key": error_key},
                        level="error",
                    )
...
class MasterModelUploadMixin(UploadMixin):
    ...
    def convert2upload_fields(self, request, row_dict, upload_fields):
        ...
        upload_model = self.upload_model if self.upload_model is not None else self.model  # type: ignore[attr-defined]
        if issubclass(upload_model, MasterModel):
            if "valid_from" not in converted:
                converted["valid_from"] = default_valid_from_date()
            if "valid_to" not in converted:
